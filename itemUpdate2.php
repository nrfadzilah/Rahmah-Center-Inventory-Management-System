...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (isset($_FILES['itemImage']) && $_FILES['itemImage']['error'] == 0) {
            ...
            $q = "SELECT itemImage FROM products WHERE itemID = $id";
        ...
        if (empty($error)) {
            $q = "SELECT itemID FROM products WHERE itemName = '$itemName' AND itemID != $id";
            $result = mysqli_query($connect, $q);
            ...
            if (mysqli_num_rows($result) == 0) {
                ...
                if (mysqli_affected_rows($connect) == 1) {
                    echo '<script>alert("The item has been updated."); window.location.href="itemList2.php";</script>';
                    exit();
                } else {
    ...
    $q = "SELECT itemName, category, quantity, itemImage FROM products WHERE itemID = $id";
    $result = mysqli_query($connect, $q);
    ...
    if ($result && mysqli_num_rows($result) == 1) {
        ...
        // Generate the form
        echo '<form action="itemUpdate2.php" method="post" enctype="multipart/form-data">
